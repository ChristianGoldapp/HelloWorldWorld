...
public class HelloWorld {
    ...
    public static void main(String[] args) {
        if(args.length == 0) main(new String[]{"Hello World!\n"});
        else if(args.length == 1 && args[0].length() > 0) main(new String[]{args[0].substring(0,1), args[0].substring(1)});
        else if(args.length == 2){
            System.out.print(args[0]);
            if(args[1].length() > 0) main(new String[]{args[1]});
        }
    }
}
